...
def exportar_excel(id):
    ...
    # 1. Preparar los datos (Lógica matemática intacta)
    parametros = ParametroEvaluacion.query.filter_by(paralelo_id=id, estado=True).order_by(ParametroEvaluacion.id).all()
    
    # Obtenemos los estudiantes y los ordenamos alfabéticamente por apellidos
    ...
    for estudiante in estudiantes:
        ...
        for param in parametros_regulares:
            ...
            suma_puntajes = sum([(Calificacion.query.filter_by(actividad_id=act.id, estudiante_id=estudiante.id, estado=True).first().puntaje if Calificacion.query.filter_by(actividad_id=act.id, estudiante_id=estudiante.id, estado=True).first() else 0.0) for act in actividades])
            nota_parametro = (suma_puntajes / len(actividades)) if len(actividades) > 0 else 0.0
            
            for act in actividades:
                calif = Calificacion.query.filter_by(actividad_id=act.id, estudiante_id=estudiante.id, estado=True).first()
                fila['detalle_actividades'][act.id] = calif.puntaje if calif else 0.0
    ...
    font_total_cat = Font(bold=True, color="0D47A1") # Azul para totales de categoría
    ...
    fill_total_cat = PatternFill(start_color="E2E8F0", end_color="E2E8F0", fill_type="solid") # Gris azulado
    ...
    # Ajustar a la nueva estructura: N°, Apellidos, Nombres, C.I., R.U. (5 columnas)
    total_cols_ws1 = 5 + len(parametros_regulares) + (1 if param_liberacion else 0) + 2 # +2 por Semestre y Final
    ...
    for i, fila in enumerate(matriz, 1):
        ...
        for col_idx in range(1, total_cols_ws1 + 1):
            ...
            aplicar_borde(celda)
            if col_idx in [2, 3]: # Apellidos y Nombres alineados a la izquierda
    ...
    ws2.freeze_panes = "F3" # Congelamos desde la columna 6 (Después de N°, Apellidos, Nombres, CI, RU)

    # Fila 1: Nombres de Estructura e Identificación
    ...
    current_col = 6
    for param in parametros_regulares:
        largo = len(param.actividades) + 1 # +1 por la columna de Total
    ...
    if param_liberacion:
        largo = len(param_liberacion.actividades) + 1 # +1 por Total Liberación
        ws2.merge_cells(start_row=1, start_column=current_col, end_row=1, end_column=current_col + largo - 1)
    ...
    # Fila 2: Subtítulos (Fechas y Totales)
    for c in range(1, 6):
        aplicar_borde(ws2.cell(row=2, column=c)) # Bordes para los congelados vacíos
    ...
    for param in parametros_regulares:
        ...
        # Columna de Total por Categoría
        celda = ws2.cell(row=2, column=current_col, value=f"Total\n({param.ponderacion} pts)")
        celda.alignment = align_center
        celda.font = font_total_cat
        celda.fill = fill_total_cat
        aplicar_borde(celda)
        ws2.column_dimensions[celda.column_letter].width = 12
        current_col += 1
